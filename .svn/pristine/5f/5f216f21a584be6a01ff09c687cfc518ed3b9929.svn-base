...
new Vue({
  ...
  methods: {
    queryProductList: function (prodOfferingId) {
      var url = 'http://10.11.16.82:8080/site/api/product/productList';
      Vue.http.get(url, {
        before (req) {
          console.log(req);
        }
    ...
    filterChecked: function (index) {
      console.log('index=' + index);
      var listfilterArr = this.$refs.listfilter;
      var values = [];
      for (let i = 0; i < listfilterArr.length; i++) {
        values = values.concat(listfilterArr[i].value());
      }
      this.filterProdects(values);
    },
    filterUnchecked: function (index) {
      console.log('index=' + index);
      var listfilterArr = this.$refs.listfilter;
      var values = [];
      for (let i = 0; i < listfilterArr.length; i++) {
    ...
    filterProdects: function (values) {
      if (this.hdTimer !== null) {
        clearTimeout(this.hdTimer);
      }
